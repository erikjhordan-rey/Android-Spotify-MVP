...
public class AudioPlayerFragment extends DialogFragment implements PlayerMVPView, SeekBar.OnSeekBarChangeListener {
    ...
    @Bind(R.id.iv_album_player)
    ImageView iv_album_player;
    @Bind(R.id.txt_track_title_player)
    TextView txt_track_title_player;
    @Bind(R.id.txt_album_title_player)
    TextView txt_album_title_player;
    @Bind(R.id.sb_time_progress_player)
    SeekBar sb_time_progress_player;
    @Bind(R.id.txt_time_start)
    TextView txt_time_start;
    @Bind(R.id.txt_time_end)
    TextView txt_time_end;
    @Bind(R.id.ib_play_player)
    ImageButton ib_play_player;
        ...
        if (trackList.get(trackPosition).album.trackImages.size() > 0) {
            for (int i = 0; i < trackList.get(trackPosition).album.trackImages.size(); i++) {
                if (trackList.get(i) != null && trackList.get(trackPosition).album.trackImages.size() > 0)
                    Picasso.with(getActivity()).load(trackList.get(trackPosition).album.trackImages.get(0).url).into(iv_album_player);
            }
        } else {
            Picasso.with(getActivity()).load("http://d2c87l0yth4zbw-2.global.ssl.fastly.net/i/_global/open-graph-default.png").into(iv_album_player);
        }
    ...
    public void onStartAudioService(String trackUrl, ServiceConnection serviceConnection) {
        ...
        if (Utils.isAudioPlayerServiceRunning(AudioPlayerService.class, getActivity()) && !isPlayerPlaying) {
        ...
        } else if (!Utils.isAudioPlayerServiceRunning(AudioPlayerService.class, getActivity())) {
            trackCurrentPosition = 0;
            getActivity().getApplicationContext().startService(serviceIntent);
        }
        if (audioPlayerService == null) {
